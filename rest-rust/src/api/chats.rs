...
fn fetch_local_messages(
    ...
    match crate::db::cursor_agent::get_chat_messages_from_agent(chat_id, params.include_content) {
        Ok(messages) if !messages.is_empty() => {
            tracing::info!("Successfully loaded {} messages from cursor-agent format", messages.len());
                ...
                if let Ok(metadata) = crate::db::cursor_agent::get_chat_metadata_from_agent(chat_id) {
                    ...
                    if is_remote_fallback {
                        if let Some(meta_obj) = meta.as_object_mut() {
                            ...
                            meta_obj.insert("note".to_string(), json!("Remote device unavailable, showing local cursor-agent copy"));
                        }
                    } else {
                        if let Some(meta_obj) = meta.as_object_mut() {
        ...
        Err(e) => {
            tracing::debug!("cursor-agent format not found ({}), trying Cursor IDE format", e);
        }
    }
        ...
        if let Ok(value_str) = conn.query_row::<String, _, _>(
            ...
            if let Ok(metadata) = serde_json::from_str::<HashMap<String, Value>>(&value_str) {
                ...
                if is_remote_fallback {
                    if let Some(meta_obj) = meta.as_object_mut() {
                        meta_obj.insert("location".to_string(), json!("local_fallback"));
                        meta_obj.insert("note".to_string(), json!("Remote device unavailable, showing local Cursor IDE copy"));
                    }
                }
...
pub async fn get_chat_metadata(
    ...
    if let Some((device_id, working_dir, device_name)) = remote_info {
        ...
        // Fetch device status
        let device_status = sqlx::query_scalar::<_, String>(
...
pub async fn get_chat_messages(
    ...
    if is_remote {
        ...
        tracing::info!("Fetching messages for remote chat {} from device {}", chat_id, device_id);
        
        // Build URL for remote messages endpoint
        let mut url = format!("{}/messages/{}?include_metadata={}&include_content={}", 
            api_endpoint, 
            chat_id,
        ...
        let remote_result = state.http_client
            .get(&url)
            .send()
            .await;
        ...
        let mut result = match remote_result {
            Ok(response) if response.status().is_success() => {
                ...
                response.json::<Value>()
                    .await
                    .map_err(|e| crate::AppError::Internal(format!("Failed to parse remote response: {}", e)))?
            }
            Ok(response) => {
