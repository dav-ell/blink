...
pub fn extract_separated_content(bubble: &HashMap<String, Value>) -> HashMap<String, Value> {
    ...
    result.insert(
        "text".to_string(),
        bubble.get("text").cloned().unwrap_or(Value::String(String::new()))
    );
...
fn extract_tool_calls(bubble: &HashMap<String, Value>) -> Option<Value> {
    ...
    if !tool_data.contains_key("name") {
        ...
        unknown.insert("description".to_string(), Value::String("incomplete data".to_string()));
        return Some(Value::Array(vec![Value::Object(unknown.into_iter().collect())]));
    }
    
    let tool_name = tool_data.get("name")?.as_str()?.to_string();
    let raw_args = tool_data.get("rawArgs").and_then(|v| v.as_str()).unwrap_or("");
    ...
    tool_call.insert(
        ...
        args.get("explanation").cloned().unwrap_or(Value::String(String::new()))
    );
    tool_call.insert(
        "command".to_string(),
        args.get("command").cloned().unwrap_or(Value::String(String::new()))
    );
    tool_call.insert("arguments".to_string(), Value::Object(args.into_iter().collect()));
    
    Some(Value::Array(vec![Value::Object(tool_call.into_iter().collect())]))
}
