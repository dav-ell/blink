...
impl RetryPolicy {
        ...
        for attempt in 0..=self.max_attempts {
            match operation().await {
                ...
                Err(err) => {
                    ...
                    if !err.is_retryable() {
                        tracing::warn!("Non-retryable error encountered: {}", err);
                        return Err(err);
                    }
                    ...
                    if attempt >= self.max_attempts {
                        tracing::error!("Operation failed after {} attempts", attempt + 1);
                        break;
                    }
    ...
    pub async fn execute_with_metadata<F, Fut, T>(&self, mut operation: F) -> RetryResult<T>
    where
        F: FnMut() -> Fut,
        Fut: Future<Output = Result<T, AppError>>,
    {
