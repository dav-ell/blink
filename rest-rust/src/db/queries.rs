        ...
        Ok(value_str) => {
            ...
            let chat_metadata = ChatMetadata {
                ...
                name: metadata_json.get("name")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
                created_at: metadata_json.get("createdAt")
                    .and_then(|v| v.as_i64()),
                created_at_iso: None, // Computed later if needed
                last_updated_at: metadata_json.get("lastUpdatedAt")
                    .and_then(|v| v.as_i64()),
                last_updated_at_iso: None, // Computed later if needed
                is_archived: metadata_json.get("isArchived")
                    .and_then(|v| v.as_bool())
                    .unwrap_or(false),
                is_draft: metadata_json.get("isDraft")
                    .and_then(|v| v.as_bool())
                    .unwrap_or(false),
                total_lines_added: metadata_json.get("totalLinesAdded")
                    .and_then(|v| v.as_i64())
                    .map(|n| n as i32)
                    .unwrap_or(0),
                total_lines_removed: metadata_json.get("totalLinesRemoved")
                    .and_then(|v| v.as_i64())
                    .map(|n| n as i32)
                    .unwrap_or(0),
                subtitle: metadata_json.get("subtitle")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
...
pub fn get_chat_messages(conn: &Connection, chat_id: &str) -> Result<Vec<(String, HashMap<String, Value>)>> {
    ...
    if include_archived {
        ...
        for row_result in rows {
            ...
            if let Ok(metadata) = serde_json::from_str::<HashMap<String, Value>>(&value_str) {
                if !metadata.get("isArchived").and_then(|v| v.as_bool()).unwrap_or(false) {
                    count += 1;
                }
            }
        }
