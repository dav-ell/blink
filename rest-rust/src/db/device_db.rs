...
pub async fn ensure_device_db_initialized(pool: &SqlitePool) -> Result<()> {
    ...
    let devices_table_info =
        sqlx::query("SELECT sql FROM sqlite_master WHERE type='table' AND name='devices'")
            .fetch_optional(pool)
            .await?;
    ...
    if let Some(row) = devices_table_info {
        ...
        if !sql.contains("api_endpoint") {
            ...
            // Drop old table
            sqlx::query("DROP TABLE devices").execute(pool).await?;
    ...
    if devices_backup_exists > 0 {
        ...
        let columns: Vec<String> =
            sqlx::query_scalar("SELECT name FROM pragma_table_info('devices_backup')")
                .fetch_all(pool)
                .await?;
        ...
        let restore_result = sqlx::query(&query).execute(pool).await;

        if let Err(e) = restore_result {
            tracing::error!(
                "Failed to restore devices from backup: {}. Starting fresh.",
                e
            );
        } else {
    ...
    let table_info =
        sqlx::query("SELECT sql FROM sqlite_master WHERE type='table' AND name='remote_chats'")
            .fetch_optional(pool)
            .await?;
    ...
    if let Some(row) = table_info {
        ...
        if sql.contains("created_at INTEGER") || sql.contains("created_at INT") {
            tracing::warn!(
                "Detected old remote_chats schema with INTEGER timestamps, migrating..."
            );
            ...
            // Drop old table
            sqlx::query("DROP TABLE remote_chats").execute(pool).await?;
