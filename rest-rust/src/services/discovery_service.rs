...
async fn device_exists_by_endpoint(pool: &SqlitePool, endpoint: &str) -> Result<bool> {
    let count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM devices WHERE api_endpoint = ? AND is_active = 1")
            .bind(endpoint)
            .fetch_one(pool)
            .await?;
...
async fn get_device_id_by_endpoint(pool: &SqlitePool, endpoint: &str) -> Result<String> {
    let device_id: String =
        sqlx::query_scalar("SELECT id FROM devices WHERE api_endpoint = ? AND is_active = 1")
            .bind(endpoint)
            .fetch_one(pool)
            .await?;
            ...
            if device_exists {
                ...
                if let Ok(device_id) = get_device_id_by_endpoint(pool, &endpoint).await {
                    ...
                    .await
                    {
                        ...
                        tracing::info!(
                            "Updated status to online for device: {} ({})",
                            host.name,
                            endpoint
                        );
                    }
