pub mod agent_service;
pub mod chat_service;
pub mod device_service;
pub mod discovery_service;
pub mod http_agent_service;
pub mod job_service;

pub use agent_service::{run_cursor_agent, AgentResponse, AVAILABLE_MODELS};
pub use chat_service::create_new_chat;
pub use device_service::{
    create_device, create_remote_chat, delete_device, get_all_devices, get_device, get_remote_chat,
    update_device_last_seen, update_device_status, update_remote_chat_metadata,
};
pub use discovery_service::{discover_remote_hosts, DiscoveryResult, DiscoveryStatus};
pub use http_agent_service::{
    execute_remote_cursor_agent, test_http_connection, HttpAgentResponse,
};
pub use job_service::{
    cleanup_old_jobs, create_job, get_chat_jobs, get_job, init_jobs_db, update_job,
    update_job_status, update_job_result,
};
